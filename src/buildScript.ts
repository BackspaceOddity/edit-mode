...
` : '';
  ...
  var active=false,pending=null,editMode='visual',copyId=null,rewriting=null,rwOrig='',rwOrigHtml='';
  ...
  /* Re-position a fixed popup fully inside the viewport once its real size is known.
     Flip below->above the anchor when below overflows the bottom; then hard-clamp top &
     left so the whole box (incl. its Send/Save buttons) stays on-screen for any anchor. */
  function clampToViewport(el,anchorRect){ requestAnimationFrame(function(){ var M=12; var h=el.offsetHeight, w=el.offsetWidth; var vh=window.innerHeight, vw=window.innerWidth; var ar=anchorRect; var top; if(ar){ var below=ar.bottom+8; var above=ar.top-8-h; if(below+h<=vh-M){ top=below; } else if(above>=M){ top=above; } else { top=parseFloat(el.style.top||String(below))||below; } } else { top=parseFloat(el.style.top||'0')||0; } top=Math.min(top, vh-h-M); if(top<M) top=M; var left=parseFloat(el.style.left||'0')||0; left=Math.min(left, vw-w-M); if(left<M) left=M; el.style.top=top+'px'; el.style.left=left+'px'; }); }
  function onPick(ev){ if(ours(ev.target))return; if(rewriting){ if(rewriting.contains(ev.target))return; ev.preventDefault(); ev.stopPropagation(); commitRewrite(); return; } if(card.style.display!=='none'){ ev.preventDefault(); ev.stopPropagation(); closeCard(); return; } ev.preventDefault(); ev.stopPropagation(); pending=ev.target; copyId=null; var r=ev.target.getBoundingClientRect(); var top=r.bottom+8; if(top+300>window.innerHeight) top=Math.max(8,r.top-300); var left=Math.min(ev.clientX,window.innerWidth-336); if(left<8)left=8; dlg.style.top=top+'px'; dlg.style.left=left+'px'; dlg.style.display='block'; var tag=pending.tagName.toLowerCase(); var cls=pending.classList[0]?'.'+pending.classList[0]:''; var tk=tokenForEl(pending); document.getElementById('em-lbl').textContent=(tk&&twLabelMap[tk]?twLabelMap[tk]+' · ':'')+tag+cls; renderBody(); clampToViewport(dlg,r); }
  ...
  function endRewrite(restore){ if(!rewriting)return; var el=rewriting; el.removeAttribute('contenteditable'); el.removeAttribute('spellcheck'); el.style.outline=''; el.style.outlineOffset=''; if(restore) el.innerHTML=rwOrigHtml; rewriting=null; rwOrigHtml=''; }
  ...
  function openCard(id){ var t=load().threads[id]; if(!t){closeCard();return;} cardId=id; var el=findEl(t); var rect=el?el.getBoundingClientRect():null; card.innerHTML=cardHtml(t); positionCard(t,rect); card.style.display='block'; clampToViewport(card); wireCard(t,el); }
